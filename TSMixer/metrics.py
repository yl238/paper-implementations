...
def mae(df):
    """df is the output forecast DataFrame from a
    Nixtla model with the following columns:
    ds, y and forecast y_hat"""
    pass
...
def compute_metrics(dataframe: pd.DataFrame) -> dict:
    ...
    dataframe["PercentageError"] = np.where(
        ...
        np.nan,
        np.minimum(1, dataframe["AbsoluteError"] / dataframe["Quantity"]),
    )
    ...
    # Weighted MAPE (wMAPE)
    dataframe["WeightedError"] = (
        dataframe["AbsoluteError"] * dataframe["RetailPrice"]
    )
    ...
    overall_bias = (
        (total_predicted - total_actual) / total_actual
        if total_actual != 0
        else np.nan
    )
    ...
    overall_weighted_mape = (
        1 - (total_weighted_error / total_weighted_actual)
