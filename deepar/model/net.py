...
class DeepAR(nn.Module):
    ...
    def forward(self, x, idx, hidden, cell):
        ...
        Returns:
            ...
            hidden ([lstm_layers, batch_size, lstm_hidden_dim]): LSTM h from time t
            cell ([lstm_layers, batch_size, lstm_hidden_dim]): LSTM c from time step t
        """
        one_hot_embed = self.embedding(idx)
        lstm_input = torch.cat((x, one_hot_embed), dim=2)
        ...
        sigma = self.distribution_sigma(
            pre_sigma
        )  # Softplux to make sure standard deviation is positive
        return torch.squeeze(mu), torch.squeeze(sigma), hidden, cell

    def init_hidden(self, input_size):
        return torch.zeros(
            self.params.lstm_layers,
            input_size,
